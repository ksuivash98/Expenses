...
export class IncomeService {
  getAll() {
    return sortByDate(databaseService.list(TABLES.income), (i) => i.date, true);
  }

  getById(id) {
    return databaseService.find(TABLES.income, id);
  }
  ...
  getTotalDistributed() {
    return sumBy(
      databaseService.list(TABLES.budgetTransactions)
        .filter((tx) => tx.type === 'distribution' && Number(tx.amount) > 0),
  ...
  async add(data) {
    ...
    await databaseService.insert(TABLES.income, row);
  ...
  async update(id, data) {
    ...
    const updated = await databaseService.update(TABLES.income, id, {
      title: String(data.title).trim(),
      source: String(data.source).trim(),
      amount,
  ...
  async remove(id) {
    ...
    await databaseService.remove(TABLES.income, id);
    return { success: true };
  }
}
