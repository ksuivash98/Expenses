...
/**
 * Создаёт WarpText в контейнере. Возвращает { destroy, update }.
 */
export function createWarpText(container, options = {}) {
  if (!container) return { destroy() {}, update() {} };

  const props = { ...DEFAULTS, ...options };
  container.classList.add('warp-text');
  container.setAttribute('role', 'img');
  container.setAttribute('aria-label', props.text);

  // Fallback без WebGL / при reduced motion: статичный текст
  const reduceMotion = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches ?? false;
  const fallback = () => {
    container.innerHTML = `<p class="brand-wordmark warp-text-fallback">${props.text}</p>`;
    return {
      destroy() {
        container.innerHTML = '';
        container.classList.remove('warp-text');
      },
      update(next = {}) {
        Object.assign(props, next);
        container.setAttribute('aria-label', props.text);
        const el = container.querySelector('.warp-text-fallback');
        if (el) {
  ...
  try {
    ...
    console.warn('WarpText: WebGL недоступен.', error);
    return fallback();
  }
  ...
  const onReducedMotion = (event) => {
    ...
    program.uniforms.uMotion.value = motionOff ? 0 : 1;
    renderOnce();
  };
  ...
  const loop = (now) => {
    ...
    program.uniforms.uPointerActive.value = motionOff ? pointer.active * 0.35 : pointer.active;
    program.uniforms.uTime.value = motionOff ? 0 : elapsed;
